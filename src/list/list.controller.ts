...
export class ListController {
  ...
  listAssessment(@Body() dto: { assessment_id: string; list_id: string }) {
    return this.listService.listAssessment(dto.list_id, dto.assessment_id);
  }

  @Post('listlesson')
  listLesson(@Body() dto: { lesson_id: string; list_id: string }) {
    return this.listService.listAssessment(dto.list_id, dto.lesson_id);
  }
  @Post('listuserdocuments')
  listUserDocuments(
    @Body() dto: { userdocuments_id: string; list_id: string },
  ) {
    return this.listService.listAssessment(dto.list_id, dto.userdocuments_id);
  }
  ...
  findOneUserList(
    ...
    return this.listService.findOnelist(list_id, user_id);
  }
