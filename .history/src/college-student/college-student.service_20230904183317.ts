...
export class CollegeStudentService {
  ...
  async storeInfo(userId, dto: CollegeStudentDTO) {
    try {
  ...
  async deleteStudentInfo(id: string) {
    try {
      await this.prisma.collegeStudentInfo.delete({
        where: {
          id: id,
        },
      });
      ...
      throw new BadRequestException('Failed to delete student info');
    }
  }
}
